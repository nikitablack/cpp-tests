...
ShapesRenderer::ShapesRenderer(UINT const bufferCount, std::shared_ptr<Window> const window) : Graphics{ bufferCount, window }
{
	...
	_viewport.Width = static_cast<float>(wSize.x);
	_viewport.Height = static_cast<float>(wSize.y);
...
void ShapesRenderer::render(UINT const numVertices)
{
	UINT presentCount;
...
void ShapesRenderer::setVertices(vector<float> const & data)
{
...
void ShapesRenderer::createConstantBuffer(shared_ptr<Window> const window)
{
	D3D11_BUFFER_DESC desc;
	ZeroMemory(&desc, sizeof(desc));
	desc.Usage = D3D11_USAGE_DYNAMIC;
	...
	XMMATRIX const matrixProjectionDX(XMMatrixOrthographicOffCenterLH(0.0f, static_cast<float>(wSize.x), static_cast<float>(wSize.y), 0.0f, 0.0f, 1.0f));
